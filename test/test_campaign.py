...
class CampaignTestCase(object):
  ...
  def test_create(self):
    ...
    self.assertEqual("\"TextUrl\": \"http://example.com/campaign.txt\"" in c.faker.actual_body, True)
  ...
  def test_summary(self):
    ...
    self.assertEqual(summary.WebVersionURL, "http://createsend.com/t/r-3A433FC72FFE3B8B")
    self.assertEqual(summary.WebVersionTextURL, "http://createsend.com/t/r-3A433FC72FFE3B8B/t")
    self.assertEqual(summary.WorldviewURL, "http://client.createsend.com/reports/wv/r/3A433FC72FFE3B8B")
  ...
  def test_lists_and_segments(self):
    ...
    self.assertEqual(ls.Lists[0].ListID, "a58ee1d3039b8bec838e6d1482a8a965")
    self.assertEqual(ls.Segments[0].Title, "Segment for campaign")
    self.assertEqual(ls.Segments[0].ListID, "2bea949d0bf96148c3e6a209d2e82060")
    self.assertEqual(ls.Segments[0].SegmentID, "dba84a225d5ce3d19105d7257baac46f")
  ...
  def test_recipients(self):
    ...
    self.assertEqual(res.Results[0].EmailAddress, "subs+6g76t7t0@example.com")
    self.assertEqual(res.Results[0].ListID, "a994a3caf1328a16af9a69a730eaa706")

  def test_opens(self):
    min_date = "2010-01-01"
    self.campaign.stub_request("campaigns/%s/opens.json?date=%s&orderfield=date&page=1&pagesize=1000&orderdirection=asc" % (self.campaign_id, urllib.parse.quote(min_date, '')), "campaign_opens.json")
    opens = self.campaign.opens(min_date)
    self.assertEqual(len(opens.Results), 5)
    self.assertEqual(opens.Results[0].EmailAddress, "subs+6576576576@example.com")
    self.assertEqual(opens.Results[0].ListID, "512a3bc577a58fdf689c654329b50fa0")
  ...
  def test_clicks(self):
    min_date = "2010-01-01"
    self.campaign.stub_request("campaigns/%s/clicks.json?date=%s&orderfield=date&page=1&pagesize=1000&orderdirection=asc" % (self.campaign_id, urllib.parse.quote(min_date, '')), "campaign_clicks.json")
    clicks = self.campaign.clicks(min_date)
    self.assertEqual(len(clicks.Results), 3)
    self.assertEqual(clicks.Results[0].EmailAddress, "subs+6576576576@example.com")
    self.assertEqual(clicks.Results[0].URL, "http://video.google.com.au/?hl=en&tab=wv")
    self.assertEqual(clicks.Results[0].ListID, "512a3bc577a58fdf689c654329b50fa0")
  ...
  def test_unsubscribes(self):
    min_date = "2010-01-01"
    self.campaign.stub_request("campaigns/%s/unsubscribes.json?date=%s&orderfield=date&page=1&pagesize=1000&orderdirection=asc" % (self.campaign_id, urllib.parse.quote(min_date, '')), "campaign_unsubscribes.json")
    unsubscribes = self.campaign.unsubscribes(min_date)
    self.assertEqual(len(unsubscribes.Results), 1)
    self.assertEqual(unsubscribes.Results[0].EmailAddress, "subs+6576576576@example.com")
    self.assertEqual(unsubscribes.Results[0].ListID, "512a3bc577a58fdf689c654329b50fa0")
  ...
  def test_spam(self):
    min_date = "2010-01-01"
    self.campaign.stub_request("campaigns/%s/spam.json?date=%s&orderfield=date&page=1&pagesize=1000&orderdirection=asc" % (self.campaign_id, urllib.parse.quote(min_date, '')), "campaign_spam.json")
    spam = self.campaign.spam(min_date)
    self.assertEqual(len(spam.Results), 1)
    self.assertEqual(spam.Results[0].EmailAddress, "subs+6576576576@example.com")
    self.assertEqual(spam.Results[0].ListID, "512a3bc577a58fdf689c654329b50fa0")
  ...
  def test_bounces(self):
    min_date = "2010-01-01"
    self.campaign.stub_request("campaigns/%s/bounces.json?date=%s&orderfield=date&page=1&pagesize=1000&orderdirection=asc" % (self.campaign_id, urllib.parse.quote(min_date, '')), "campaign_bounces.json")
    bounces = self.campaign.bounces(min_date)
    self.assertEqual(len(bounces.Results), 2)
    self.assertEqual(bounces.Results[0].EmailAddress, "asdf@softbouncemyemail.com")
    self.assertEqual(bounces.Results[0].ListID, "654523a5855b4a440bae3fb295641546")
    self.assertEqual(bounces.Results[0].BounceType, "Soft")
    self.assertEqual(bounces.Results[0].Date, "2010-07-02 16:46:00")
    self.assertEqual(bounces.Results[0].Reason, "Bounce - But No Email Address Returned ")
