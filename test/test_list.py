...
class ListTestCase(object):
  ...
  def test_update_custom_field(self):
    key = "[mycustomfield]"
    self.list.stub_request("lists/%s/customfields/%s.json" % (self.list.list_id, urllib.parse.quote(key)),
  ...
  def test_delete_custom_field(self):
    ...
    self.list.stub_request("lists/%s/customfields/%s.json" % (self.list.list_id, urllib.parse.quote(custom_field_key)), None)
    self.list.delete_custom_field(custom_field_key)

  def test_update_custom_field_options(self):
    custom_field_key = "[newdatefield]"
    new_options = [ "one", "two", "three" ]
    self.list.stub_request("lists/%s/customfields/%s/options.json" % (self.list.list_id, urllib.parse.quote(custom_field_key)), None)
  ...
  def test_segments(self):
    ...
    self.assertEqual(segments[0].ListID, 'a58ee1d3039b8bec838e6d1482a8a965')
    self.assertEqual(segments[0].SegmentID, '46aa5e01fd43381863d4e42cf277d3a9')
  ...
  def test_active(self):
    min_date = "2010-01-01"
    self.list.stub_request("lists/%s/active.json?date=%s&orderfield=email&page=1&pagesize=1000&orderdirection=asc" % (self.list.list_id, urllib.parse.quote(min_date)), "active_subscribers.json")
    ...
    self.assertEqual(res.Results[0].EmailAddress, "subs+7t8787Y@example.com")
    ...
    self.assertEqual(res.Results[0].CustomFields[0].Value, "http://example.com")
    self.assertEqual(res.Results[0].CustomFields[1].Key, "multi select field")
    self.assertEqual(res.Results[0].CustomFields[1].Value, "option one")
    self.assertEqual(res.Results[0].CustomFields[2].Key, "multi select field")
    self.assertEqual(res.Results[0].CustomFields[2].Value, "option two")
    self.assertEqual(res.Results[0].ReadsEmailWith, "Gmail")

  def test_unconfirmed(self):
    min_date = "2010-01-01"
    self.list.stub_request("lists/%s/unconfirmed.json?date=%s&orderfield=email&page=1&pagesize=1000&orderdirection=asc" % (self.list.list_id, urllib.parse.quote(min_date)), "unconfirmed_subscribers.json")
  ...
  def test_unsubscribed(self):
    min_date = "2010-01-01"
    self.list.stub_request("lists/%s/unsubscribed.json?date=%s&orderfield=email&page=1&pagesize=1000&orderdirection=asc" % (self.list.list_id, urllib.parse.quote(min_date)), "unsubscribed_subscribers.json")
    ...
    self.assertEqual(res.NumberOfPages, 1)
    self.assertEqual(len(res.Results), 5)
    self.assertEqual(res.Results[0].EmailAddress, "subscriber@example.com")
  ...
  def test_deleted(self):
    min_date = "2010-01-01"
    self.list.stub_request("lists/%s/deleted.json?date=%s&orderfield=email&page=1&pagesize=1000&orderdirection=asc" % (self.list.list_id, urllib.parse.quote(min_date)), "deleted_subscribers.json")
  ...
  def test_bounced(self):
    min_date = "2010-01-01"
    self.list.stub_request("lists/%s/bounced.json?date=%s&orderfield=email&page=1&pagesize=1000&orderdirection=asc" % (self.list.list_id, urllib.parse.quote(min_date)), "bounced_subscribers.json")
    ...
    self.assertEqual(res.Results[0].EmailAddress, "bouncedsubscriber@example.com")
