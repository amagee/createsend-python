...
class SubscriberTestCase(object):
  ...
  def test_get(self):
    ...
    self.subscriber.stub_request("subscribers/%s.json?email=%s" % (self.list_id, urllib.parse.quote(email)), "subscriber_details.json")
    ...
    self.assertEqual(subscriber.CustomFields[0].Value, 'http://example.com')
  ...
  def test_update_with_custom_fields(self):
    ...
    self.subscriber.stub_request("subscribers/%s.json?email=%s" % (self.list_id, urllib.parse.quote(self.subscriber.email_address)), None)
    custom_fields = [ { "Key": 'website', "Value": 'http://example.com/' } ]
  ...
  def test_import_subscribers_partial_success(self):
    ...
    self.assertEqual(import_result.FailureDetails[0].EmailAddress, "example+1@example")
    self.assertEqual(import_result.FailureDetails[0].Code, 1)
    self.assertEqual(import_result.FailureDetails[0].Message, "Invalid Email Address")
    self.assertEqual(import_result.TotalUniqueEmailsSubmitted, 3)
  ...
  def test_history(self):
    self.subscriber.stub_request("subscribers/%s/history.json?email=%s" % (self.list_id, urllib.parse.quote(self.subscriber.email_address)), "subscriber_history.json")
