...
class CreateSendTestCase(object):
  ...
  def test_that_default_user_agent_is_set(self):
    ...
    cl = self.cs.clients()
    self.assertEqual(self.cs.headers['User-Agent'], CreateSend.default_user_agent)
  ...
  def test_administrators(self):
  	...
  	self.assertEqual('admin1@blackhole.com', administrators[0].EmailAddress)
  ...
  def test_set_primary_contact(self):
    ...
    self.cs.stub_request('primarycontact.json?email=%s' % urllib.parse.quote(email, ''), 'admin_set_primary_contact.json')
  ...
  def test_external_session_url(self):
    ...
    self.assertEqual("https://external1.createsend.com/cd/create/ABCDEF12/DEADBEEF?url=FEEDDAD1", result.SessionUrl)
  ...
  def test_bad_request(self):
    ...
    try:
      ...
      self.assertEqual('The CreateSend API responded with the following error - 98798: A crazy API error', "%s" % br)
  ...
  def test_unauthorized(self):
    ...
    try:
      ...
      self.assertEqual('The CreateSend API responded with the following error - 98798: A crazy API error', "%s" % ua)
